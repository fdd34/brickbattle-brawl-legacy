...
function ApplyWindEffect(dir,loc)
	if dir and dir:FindFirstChild("Torso") and dir:FindFirstChild("Forcefield") == nil and loc then
		local dirTorso = dir:FindFirstChild("Torso")
		local force = Instance.new("BodyVelocity",dirTorso)
		force.Name = "WindEffect"
		force.maxForce = Vector3.new(1e7, 1e7, 1e7)
		force.P = 125
		force.velocity =  (loc * 150) + Vector3.new(0,30,0)
...
function OnHit(hit)
	if hit and hit.Parent and hit.Parent:FindFirstChild("Humanoid") then
		...
		if hitHum ~= Humanoid and hitHum.Health > 0 and Damage > 0 then
			...
			local d = Torso.CFrame.lookVector
			ApplyWindEffect(hit.Parent,Vector3.new(d.x, d.y, d.z))
			TagHumanoid(hitHum)
