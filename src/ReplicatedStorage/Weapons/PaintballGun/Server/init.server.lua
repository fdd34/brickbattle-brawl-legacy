--------------------------------------------------------------------------------------------------------------------------------------------------------------
-- RbxClient Paintball Gun [Server]
--------------------------------------------------------------------------------------------------------------------------------------------------------------
...
-- Other Variables
local Networking = {}
local VELOCITY = 90 -- constant
local Equipped = false
local KillIcon = 1160900511
...
function fire(mouse_pos)
	if Character and Player and Head then
		...
		local g = (-9.81 * 16) -- original: (-9.81 * 20)
...
function ChangeSkin(skin)
	if skin == "Classic" then
		...
		if SkinsModule.Paintball[skin] then
			Skin.Mesh.TextureId = "rbxassetid://"..SkinsModule.Paintball[skin]
		else
...
Tool.Equipped:connect(function()
	...
	ImportantConnection = PlayerInfo.Skins.PaintballSkin.Changed:connect(ChangeSkin)
	SlingshotSound:Play()
