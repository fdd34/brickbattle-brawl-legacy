...
local BLAST_DAMAGE = 140 -- Amount of damage done to players
...
local function ApplyTags(target)
	...
	DebrisService:AddItem(IconTagClone, 3)
	DebrisService:AddItem(creatorTagClone, 3)
end
	...
	if hitPart and hitDistance then
		...
		if humanoid and humanoid.Health > 0 then -- Humanoids are tagged and damaged
			if hitPart.Name == 'Torso' then
				...
				local blastForce = Instance.new('BodyForce') --NOTE: We will multiply by mass so bigger parts get blasted more
				distanceFactor = 1 - distanceFactor
				if CreatorTag.Value.Name == hitPart.Parent.Name then
					BLAST_DAMAGE = BLAST_DAMAGE / 2
					blastForce.force = (hitPart.Position - blastCenter).unit * BLAST_FORCE * hitPart:GetMass()
				else
					blastForce.force = (hitPart.Position - blastCenter).unit * (BLAST_FORCE / 1.2) * hitPart:GetMass()
				end
				blastForce.Parent = hitPart
				humanoid:TakeDamage(BLAST_DAMAGE * distanceFactor)
				game:GetService("Debris"):AddItem(blastForce, .2)
			end
